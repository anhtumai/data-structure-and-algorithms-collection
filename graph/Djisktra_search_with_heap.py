...
def initialize(graph: Graph, start: any) -> tuple[Distancedict, Parentdict, MinHeap]:
    ...
    min_heap = MinHeap([Node(node_name, distances[node_name])
                        for node_name in node_names if node_name != start])
...
def get_distances_and_parents(graph: Graph, start: any) -> tuple[Distancedict, Parentdict]:
...
def djisktra(graph: Graph, start: any, end: any) -> list[any]:
    ...
    _, parents = get_distances_and_parents(graph)
